    ...
    <article className={`rounded-[1.75rem] border p-6 shadow-sm ${toneStyles}`}>
      <p
        className={`text-xs font-semibold uppercase tracking-[0.22em] ${
          tone === 'primary' ? 'text-sky-200' : tone === 'hold' ? 'text-amber-700' : 'text-slate-500'
        }`}
      >
        {badge}
      </p>
      <h3 className="mt-3 text-2xl font-semibold tracking-tight">{title}</h3>
      <p className={`mt-3 text-sm leading-7 ${mutedText}`}>{summary}</p>

      <ul className={`mt-5 space-y-2 text-sm leading-7 ${mutedText}`}>
